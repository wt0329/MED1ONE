...
$("#rstInsert").on("click",function(){
	$(".examList").eq(0).val("23.6"); //신체계측
	$(".examList").eq(1).val("61점"); //체성분검사
	$(".examList").eq(2).val("평균"); //생체 나이 측정
	$(".examList").eq(3).val("1.0/1.2 "); // 안과검사
	$(".examList").eq(4).val("특별한 치료는 불필요"); //폐기능 검사
	$(".examList").eq(5).val("4.09"); //심전도
	$(".examList").eq(6).val("23dB "); //청력검사
	$(".examList").eq(7).val("결과 확인 필요"); //일반혈액검사
	$(".examList").eq(8).val("정상"); // 요검사
});
...
$(document).on("click", "#saveRstBtn", function(){
	...
	rstTextarea = $('#exampleFormControlTextarea2').val();
